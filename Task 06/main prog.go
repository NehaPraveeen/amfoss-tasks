...
func main() {
	fmt.Println("--------Pirate King's Scheduler Simulator--------")
	...
	for i := 0; i < len(crews); i++ {
		if currentTime < crews[i].ArrivalTime {
			currentTime = crews[i].ArrivalTime
		}
		
		startTime := currentTime
		currentTime += crews[i].BurstTime

		crews[i].CompletionTime = currentTime
		...
		totalTurnaround += crews[i].TurnaroundTime

		fmt.Printf("[%s: %d-%d] ", crews[i].ID, startTime, currentTime)
	}

	fmt.Println("\n\nCrew ID\t\tArrival\tBurst\tWaiting\tTurnaround")
	for i := 0; i < len(crews); i++ {
	...
	fmt.Printf("Average Turnaround Time: %.2f hours\n", float64(totalTurnaround)/numCrews)
}
